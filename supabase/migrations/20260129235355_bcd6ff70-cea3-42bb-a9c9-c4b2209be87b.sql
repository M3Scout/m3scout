-- Update the unified_player_season_stats view to use manual_player_stats instead of player_stats
-- This ensures proper source separation: live data from match_player_stats, manual from manual_player_stats
...
AS
WITH live_stats AS (
  -- Aggregate stats from live matches (match_player_stats + match_players)
  SELECT
    mp.player_id,
    m.season_year,
    m.competition_id,
...
manual_stats AS (
  -- Stats from manual_player_stats (external games not tracked via Live Match)
  SELECT
    mps.player_id,
    mps.season_year,
    mps.competition_id,
...
-- PRIORITY: Use live data when available, fallback to manual only if no live exists
SELECT
  ...
  -- Use live data if exists, otherwise manual
