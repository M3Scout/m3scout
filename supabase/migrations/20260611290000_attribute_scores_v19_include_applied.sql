-- calculate_player_attribute_scores v19
-- Única mudança vs v18: src_live agora inclui status IN ('finished','applied').
--
-- Motivo: partidas com status 'applied' não estão necessariamente consolidadas
-- em player_stats. No caso de Vitor, os 66 min do único jogo ao vivo têm
-- status='applied' mas NÃO existem em player_stats (que tem 1494 min).
-- Filtrar apenas 'finished' excluía esses minutos e distorcia o cálculo.
-- O frontend usa exatamente status IN ('finished','applied'), seguimos o mesmo critério.
...
DECLARE
  ...
  v_s_assists numeric; v_s_accurate_passes numeric; v_s_long_passes_accurate numeric;
  v_s_long_passes_failed_neg numeric; v_s_successful_dribbles numeric;
  v_s_dribbles_failed_neg numeric; v_s_fouls_drawn numeric; v_s_passes_failed_neg numeric;
...
BEGIN
  ...
  v_per90 := jsonb_build_object(
    'goals', v_goals_p90, 'shots_on_target', v_shots_on_target_p90,
    'shots_on_post', v_shots_on_post_p90, 'penalties_won', v_penalties_won_p90,
    'offsides', v_offsides_p90, 'key_passes', v_key_passes_p90,
  ...
  INSERT INTO public.player_attribute_scores (
    ...
    jsonb_build_object(
      ...
      'engine_version', 'v19',
      'data_source', 'src_ps (player_stats) UNION ALL src_live (match_player_stats finished+applied) + player_field_presence minutes',
      'schema_notes', 'live: passes_total=failed, dribbles_total=failed, duels_total=lost, aerial_duels_total=lost; ps: totais reais'
    ),
    now()
  );
