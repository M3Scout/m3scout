-- Fix: coeficiente de competição multiplicado por 90 (era 20).
-- Com coeffs entre 0.65–1.12, o range útil passa a ser ~59–100 pontos.
...
BEGIN
  ...
  -- Nível da competição: coeff * 90 (coeffs 0.65–1.12 → range ~59–100)
  SELECT LEAST(COALESCE(SUM(final_coefficient * minutes) / NULLIF(SUM(minutes),0), 0.65) * 90, 100)
  INTO v_comp_level1
  FROM public.unified_player_season_stats
  WHERE player_id = p_player_id AND season_year = v_year1 AND minutes > 0;
  v_comp_level1 := COALESCE(v_comp_level1, 58); -- fallback: ~coeff mínimo * 90
  ...
  IF v_year2 IS NOT NULL THEN
    ...
    v_comp_level2 := COALESCE(v_comp_level2, 58);
  END IF;
...
-- Apenas a linha v_competition_level_score muda de * 20 para * 90.
...
BEGIN
  ...
  LOOP
    ...
    -- Nível da competição: coeff * 90 (corrigido de * 20)
    v_competition_level_score := LEAST(v_comp_record.final_coefficient * 90, 100);
