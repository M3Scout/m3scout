-- Adiciona suporte a "Pênalti Sofrido" no fluxo de Jogo Ao Vivo.
--
-- 1. Coluna em match_player_stats (per-game tracking)
-- 2. Novo valor no enum match_event_type
-- 3. Atualiza apply_event_stats() para mapear 'penalty_won' → 'penalties_won'
-- 4. Recria unified_player_season_stats incluindo penalties_won

ALTER TABLE public.match_player_stats
  ADD COLUMN IF NOT EXISTS penalties_won INTEGER NOT NULL DEFAULT 0;

ALTER TYPE public.match_event_type ADD VALUE IF NOT EXISTS 'penalty_won';

-- Atualiza a função de aplicação de eventos para incluir o novo tipo
...
-- Recria a view unificada incluindo penalties_won em ambas as fontes
...
WITH presence_minutes AS (
  ...
  SELECT mp.player_id,
    ...
    sum(COALESCE(mps.penalties_won, 0)) AS penalties_won,
...
SELECT COALESCE(l.player_id, m.player_id) AS player_id,
  ...
  COALESCE(l.penalties_won, m.penalties_won::bigint) AS penalties_won,
