-- Update create_live_event_v2 to allow official events in finished/applied matches when p_force_time_seconds is provided (review mode)
CREATE OR REPLACE FUNCTION public.create_live_event_v2(
  p_game_id uuid, 
  p_player_id uuid, 
  p_type text, 
  p_half integer DEFAULT NULL::integer, 
  p_notes text DEFAULT NULL::text, 
  p_force_time_seconds integer DEFAULT NULL::integer
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Block if match status is locked
  IF v_match.status = 'locked' THEN
    RETURN json_build_object('success', false, 'message', 'Partida bloqueada (LOCKED). Contate um administrador.');
  END IF;

  -- Determine period from p_half or match state
  v_period := COALESCE(p_half, v_match.half, 1);

  -- Determine event status based on match status and context
  -- If match is live, events are official
  -- If match is finished/applied but force_time_seconds is provided (review mode), events are also official
  IF v_match.status = 'live' THEN
    v_event_status := 'official';
  ELSIF v_match.status IN ('finished', 'applied', 'completed') AND p_force_time_seconds IS NOT NULL THEN
    -- Review mode: allow official events with forced time
    v_event_status := 'official';
  ELSE
