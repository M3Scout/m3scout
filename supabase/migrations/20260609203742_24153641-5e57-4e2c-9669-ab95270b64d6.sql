-- 1) Add penalties_won column to player_stats and match_player_stats
...
-- 2) Add 'penalty_won' to match_event_type enum (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_enum e
    JOIN pg_type t ON t.oid = e.enumtypid
    WHERE t.typname = 'match_event_type' AND e.enumlabel = 'penalty_won'
  ) THEN
    ALTER TYPE public.match_event_type ADD VALUE 'penalty_won';
  END IF;
END $$;

-- 3) Update apply_event_stats to map 'penalty_won' -> penalties_won
...
-- 4) Recreate unified_player_season_stats view to expose penalties_won
