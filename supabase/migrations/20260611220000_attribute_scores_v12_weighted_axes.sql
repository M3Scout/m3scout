-- calculate_player_attribute_scores v12
-- Substitui médias simples por médias ponderadas em todos os eixos.
-- Métricas situacionais (penalties_won, crosses_failed_neg, etc.) têm peso menor
-- para não achatar os scores de atletas cujos números naturais são baixos nessas stats.
-- Pesos somam 1.0 em cada eixo.
...
BEGIN
  ...
  -- ── Sub-scores individuais (normalização por target) ──────────────────────
  -- ATA
  ...
  v_def := (v_s_tackles * 0.30) + (v_s_interceptions * 0.25) + (v_s_clearances * 0.15) + (v_s_blocked_shots * 0.15) + (v_s_times_dribbled_neg * 0.15);

  v_tat := (v_s_recoveries * 0.25) + (v_s_ground_duels_won * 0.15) + (v_s_aerial_duels_won * 0.15) + (v_s_ground_duels_lost_neg * 0.10) + (v_s_aerial_duels_lost_neg * 0.10) + (v_s_fouls_committed_neg * 0.10) + (v_s_yellow_neg * 0.05) + (v_s_red_neg * 0.10);
  ...
  INSERT INTO public.player_attribute_scores (
    ...
    jsonb_build_object(
      ...
      'weights', jsonb_build_object(
        'ata', '0.45/0.30/0.10/0.05/0.10 (goals/shots_on_target/shots_on_post/penalties_won/offsides_neg)',
        'cri', '0.35/0.25/0.20/0.15/0.05 (chances_created/key_passes/progressive_passes/crosses_success/crosses_failed_neg)',
        'tec', '0.20/0.15/0.15/0.15/0.10/0.05/0.05/0.05/0.05/0.05 (accurate_passes/assists/dribbles/prog_passes/long_pass_acc/fouls_drawn/passes_neg/long_pass_neg/dribbles_neg/poss_neg)',
        'def', '0.30/0.25/0.15/0.15/0.15 (tackles/interceptions/clearances/blocked_shots/dribbled_neg)',
        'tat', '0.25/0.15/0.15/0.10/0.10/0.10/0.05/0.10 (recoveries/gd_won/ad_won/gd_lost_neg/ad_lost_neg/fouls_comm_neg/yellow_neg/red_neg)'
      )
    ),
    now()
  );
...
-- Recalcula todos os scores com o motor v12
