-- v24: usa mp.minutes_played em vez de player_field_presence
-- Problema v23: player_field_presence tinha intervalos com bug (ex: 229 min em 1 jogo)
-- Problema v23: COUNT(DISTINCT match_id) contava jogos com 0 min (filtro WHERE minutes > 0
--               era no agregado total, não por linha individual)
-- Fix 1: remove CTE presence, usa mp.minutes_played diretamente
-- Fix 2: AND COALESCE(mp.minutes_played, 0) > 0 no WHERE de src_live
...
BEGIN
  ...
  src_live AS (
    SELECT
      ...
      SUM(mp.minutes_played::numeric)::numeric     AS minutes,
  ...
  INSERT INTO public.player_attribute_scores (
    ...
    jsonb_build_object(
      ...
      'engine_version', 'v24',
      'data_source',    'player_stats UNION ALL src_live(minutes_played); live excluído só onde is_live_correction=true'
    ),
    now()
  );
