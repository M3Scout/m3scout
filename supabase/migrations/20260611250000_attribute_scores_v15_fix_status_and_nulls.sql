-- calculate_player_attribute_scores v15
-- Correções críticas no src_live:
--
-- 1. STATUS: era IN ('finished','applied') — partidas 'applied' já foram consolidadas
--    em player_stats (src_ps), causando duplicação de minutos no denominador.
--    Corrigido para = 'finished' apenas.
--
-- 2. ZEROS PREGUIÇOSOS → NULL ou coluna real:
--    - recoveries: coluna EXISTE em match_player_stats → SUM(mps.recoveries)
--    - long_passes_accurate: não existe no live → NULL (ignorado pelo SUM)
--    - long_passes_total:    não existe no live → NULL (ignorado pelo SUM)
--    - times_dribbled_past:  não existe no live → NULL (ignorado pelo SUM)
...
BEGIN
  WITH src_ps AS (
    -- Estatísticas manuais (já consolidadas — inclui jogos com status 'applied')
    SELECT
      COUNT(*)                                                AS matches,
  ...
  src_live AS (
    -- Dados ao vivo: apenas partidas 'finished' (não 'applied' — já estão em src_ps).
    -- Colunas ausentes em match_player_stats usam NULL para não diluir o per-90:
    --   long_passes_accurate / long_passes_total / times_dribbled_past → NULL
    -- Colunas presentes mapeadas corretamente:
    --   recoveries → SUM(mps.recoveries)
    --   shots_blocked (live) → aliasado como blocked_shots
    SELECT
      ...
      NULL                                                    AS long_passes_accurate,
      NULL                                                    AS long_passes_total,
...
-- Recalcula todos os scores com o motor v15
