-- calculate_player_attribute_scores v18
-- Restaura UNION ALL (src_ps + src_live) mantendo as correções semânticas da v17.
--
-- src_ps : player_stats (dados manuais consolidados) — sem alterações vs v13
-- src_live: match_player_stats (apenas status='finished' para não duplicar com src_ps)
--           com inversões do schema corrigidas:
--             passes_total    = FALHADOS → total real = passes_completed + passes_total
--             dribbles_total  = FALHADOS → total real = dribbles_success + dribbles_total
--             duels_total     = PERDIDOS → total real = duels_won + duels_total
--             aerial_duels_total = PERDIDOS → total real = aerial_duels_won + aerial_duels_total
--             ground_duels    = derivado: total_duelos - duelos_aéreos
--             was_dribbled    = times_dribbled_past (coluna existe)
--             recoveries      = SUM(mps.recoveries) (coluna existe, não mais 0)
--             long_passes_*   = NULL::bigint (não existem no live)
--
-- Minutos em src_live: player_field_presence (preciso) com fallback mp.minutes_played
...
BEGIN
  ...
  src_ps AS (
    -- Dados manuais consolidados: coluna já armazena totais reais, sem inversões.
    SELECT
  ...
  src_live AS (
    -- Partidas ao vivo ainda NÃO consolidadas em player_stats (status='finished').
    -- Aplica inversões semânticas do schema de match_player_stats.
    SELECT
      ...
      -- duelos terrestres: derivado de total - aéreos (duels_total = PERDIDOS, aerial_duels_total = PERDIDOS)
  ...
  INSERT INTO public.player_attribute_scores (
    ...
    jsonb_build_object(
      ...
      'engine_version', 'v18',
      'data_source',    'src_ps (player_stats) UNION ALL src_live (match_player_stats finished only) + player_field_presence minutes',
      'schema_notes',   'live: passes_total=failed, dribbles_total=failed, duels_total=lost, aerial_duels_total=lost; ps: colunas já são totais reais',
...
-- Recalcula todos os scores com o motor v18
