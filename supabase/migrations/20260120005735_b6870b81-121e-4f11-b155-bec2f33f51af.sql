-- Fix apply_event_stats calls in create_live_event_v2 and void_live_event
-- The correct signature is: apply_event_stats(p_delta integer, p_event_type text, p_match_id uuid, p_player_id uuid)
-- We were calling with wrong order: (match_id, player_id, type, delta)

-- Recreate create_live_event_v2 with correct call order
...
BEGIN
  ...
  -- FIXED: Correct parameter order for apply_event_stats
  -- Signature is: apply_event_stats(p_delta integer, p_event_type text, p_match_id uuid, p_player_id uuid)
  IF v_event_status = 'official' THEN
    PERFORM apply_event_stats(1, p_type, p_game_id, p_player_id);
  END IF;
...
-- Recreate void_live_event with correct call order
...
BEGIN
  ...
  IF v_event.count_in_stats AND v_event.event_status = 'official' THEN
    PERFORM apply_event_stats(-1, v_event.event_type::text, v_event.match_id, v_event.player_id);
