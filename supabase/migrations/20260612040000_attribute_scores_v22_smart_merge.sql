-- v22: merge inteligente player_stats + match_player_stats sem double counting
-- Regra: para cada competição, usa player_stats se existir row; caso contrário usa match_player_stats
-- is_live_correction=true = live editado manualmente → já está em player_stats, não pega do live
-- Competições sem row em player_stats (ex: Corona) → pega do match_player_stats
...
BEGIN
  WITH
  -- Competições que já têm row em player_stats (manual ou live corrigido)
  -- Para essas, NÃO busca do match_player_stats
  covered_competitions AS (
  ...
  -- Fonte 1: player_stats (manual + live editado)
  src_ps AS (
    SELECT
  ...
  -- Fonte 2: match_player_stats apenas para competições SEM row em player_stats
  presence AS (
    SELECT match_id,
  ...
  src_live AS (
    ...
    WHERE mp.player_id = p_player_id
      ...
      AND m.competition_id NOT IN (SELECT competition_id FROM covered_competitions)
  ),
  ...
  INSERT INTO public.player_attribute_scores (
    ...
    jsonb_build_object(
      ...
      'engine_version', 'v22',
      'data_source',    'player_stats UNION src_live (apenas competições sem row em player_stats)'
    ),
    now()
  );
