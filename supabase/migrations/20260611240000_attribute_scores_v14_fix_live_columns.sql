-- calculate_player_attribute_scores v14
-- Corrige colunas inexistentes no src_live (match_player_stats):
--   passes_failed       → usa passes_total diretamente
--   long_passes_*       → não existem no live, usa 0
--   dribbles_failed     → usa dribbles_total diretamente
--   blocked_shots       → coluna é shots_blocked no live
--   was_dribbled        → não existe no live, usa 0
--   duels_lost          → usa duels_total diretamente
--   aerial_duels_lost   → usa aerial_duels_total diretamente
...
BEGIN
  WITH src_ps AS (
    -- Estatísticas manuais: recoveries da coluna recoveries, steals da coluna steals
    SELECT
      COUNT(*)                                                AS matches,
  ...
  src_live AS (
    -- Dados ao vivo — apenas colunas que existem em match_player_stats.
    -- long_passes_*, was_dribbled não existem no live → 0.
    -- passes_total, dribbles_total, duels_total, aerial_duels_total são diretos.
    -- shots_blocked (live) = blocked_shots (manual alias).
    SELECT
      ...
      0                                                       AS long_passes_accurate,
      0                                                       AS long_passes_total,
  ...
  INSERT INTO public.player_attribute_scores (
    ...
    jsonb_build_object(
      ...
      'engine_version', 'v14',
      'data_sources',   'player_stats (steals+recoveries) + match_player_stats (steals only)',
...
-- Recalcula todos os scores com o motor v14
