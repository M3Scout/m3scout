-- calculate_player_attribute_scores v10
-- Remove confidence multiplier: scores now reflect true per-90 performance.
-- Confidence is stored as metadata only (not used to scale scores down).
...
BEGIN
  -- Aggregate from both manual stats and live match stats
  WITH src_ps AS (
    SELECT
      COUNT(*)                                                AS matches,
  ...
  -- v10: confidence is metadata only, does NOT scale scores
  ...
  -- v10: clamp only, no confidence scaling
...
-- Recalculate all stored scores with the new engine
