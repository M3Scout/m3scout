...
export function AthleteHeroSection({ player }: AthleteHeroSectionProps) {
  ...
  return (
    <section className="pt-6 pb-[60px] relative" id="identidade">
      ...
      {/* Hero grid: info LEFT · photo RIGHT  — matches Vanilla .hero-grid */}
      <div className="grid md:grid-cols-[1fr,minmax(300px,380px)] gap-14 items-center">

        {/* ══════════════ LEFT — IDENTIDADE ══════════════ */}
        <div className="hero-l min-w-0 relative">

          {/* Ghost number — absolute behind all content, text-stroke only */}
          {player.age && (
            <div
              aria-hidden="true"
              className="absolute z-0 pointer-events-none select-none font-display font-bold leading-[0.8] tracking-[-0.04em] text-transparent"
              style={{
          ...
          {/* Content sits on z-10, above the ghost number */}
        ...
        {/* ══════════════ RIGHT — PHOTO ══════════════ */}
        <div className="rv hero-r" data-d="2">
          <div className="photo-wrap">
            {/* .photo-frame — 4/5 aspect, border, rounded-[8px] */}
            <div className="photo-frame relative aspect-[4/5] border border-white/15 rounded-[8px] overflow-hidden bg-[#191822] group">
              ...
              {/* .photo-frame::after — bottom gradient fade */}
              <div
                className="absolute inset-0 pointer-events-none z-[2]"
                style={{
                  background: "linear-gradient(180deg, transparent 55%, rgba(12,11,13,0.78) 100%)",
                }}
              />

              {/* .tick corners — accent L-brackets */}
              ...
              {/* .photo-meta — position badge bottom-left */}
