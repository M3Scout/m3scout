...
// ============ MOBILE BOTTOM NAV ============
const BOTTOM_NAV_ITEMS_INTERNAL: { href: string; label: string; module: string; icon: LucideIcon }[] = [
  { href: "/app", label: "Home", module: "app", icon: LayoutDashboard },
  { href: "/app/players", label: "Atletas", module: "players", icon: Users },
  { href: "/app/live-match", label: "Ao Vivo", module: "live_match", icon: Radio },
  { href: "/app/reports", label: "Relatórios", module: "reports", icon: FileText },
  { href: "/app/market/targets", label: "Targets", module: "players", icon: Crosshair },
];

const BOTTOM_NAV_ITEMS_PLAYER: { href: string; label: string; module: string; icon: LucideIcon }[] = [
  { href: "/app", label: "Home", module: "app", icon: LayoutDashboard },
  { href: "/app/my-profile", label: "Perfil", module: "players", icon: User },
  { href: "/app/live-match", label: "Jogos", module: "live_match", icon: Gamepad2 },
  { href: "/app/reports", label: "Relatórios", module: "reports", icon: FileText },
];
...
const MobileBottomNav = memo(function MobileBottomNav({ currentPath, can, isPlayerRole }: MobileBottomNavProps) {
  const items = isPlayerRole ? BOTTOM_NAV_ITEMS_PLAYER : BOTTOM_NAV_ITEMS_INTERNAL;
  ...
  return (
    ...
    >
      {items
        .filter(item => item.module === "app" || can(item.module as ModuleKey, "view"))
        .map(item => {
          const Icon = item.icon;
          ...
          return (
            <Link
              key={item.href}
              to={item.href}
              className="flex flex-col items-center justify-center gap-0.5 py-2 flex-1 transition-colors duration-100"
              style={{
                color: active ? "#e63946" : "rgba(255,255,255,0.35)",
              }}
            >
